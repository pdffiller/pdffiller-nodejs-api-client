...
function makeUrl(options) {
  ...
  if (options.ids) {
    endPoint = replaceEndpointVars(endPoint, options.ids); // eslint-disable-line
  }
  let url = endpoints.BASE_URL + endPoint;
...
function replaceEndpointVars(endPoint, ids) {
  Object.keys(ids).forEach((key) => {
    endPoint = endPoint.replace(`{${key}}`, ids[key]); // eslint-disable-line
  });
