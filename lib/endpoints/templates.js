...
class Templates extends Base {
   ...
   * @param {number} template_id Id of template
   * @param {object} copy_info object with copy params
   * @param {number} copy_info.folder_id folder id
   * @param {string} copy_info.name Name created for a newly filled document
   * @param {string} copy_info.action have to be copy for copy template
   * @param {function} [callback] callback function
   * @returns {Promise|void}
   */
  copy(template_id, copy_info, callback) {
    return this.request({
      method: methods.POST,
      endPoint: endpoints.TEMPLATES_BY_ID_ENDPOINT,
      ids: { template_id },
   ...
   * @param {number} template_id Id of template
   * @param {object} [search_params] object with copy params
   * @param {number} search_params.scale Scale of preview pages. Min value - 10, max value - 500.
   * @param {number} search_params.max_width Max width of preview pages. Min value - 10, max value - 3000.
   * @param {number} search_params.max_height Max height of preview pages. Min value - 10, max value - 3000.
   * @param {array} search_params.pages Page numbers of the document template. Max length = 10. Default: 1
   * @param {function} [callback] callback function
   * @returns {Promise|void}
   */
  preview(template_id, search_params, callback) {
    return this.request({
      endPoint: endpoints.TEMPLATES_PREVIEW,
      ids: { template_id },
      qs: search_params,
    }, callback);
  }
   ...
   * @param {string} fillable_info.action have to be fill for fillTemplate
   * @param {function} [callback] callback function
   * @returns {Promise|void}
   */
